...
void Light::toShader(unsigned int shaderID, glm::mat4 view)
{
    ...
    for (unsigned int i = 0; i < numLights; i++)
    {
        ...
        // Convert position/direction to view space
        ...
        glUniform3fv(glGetUniformLocation(shaderID, ("lightDirections[" + idx + "]").c_str()), 1, &VSLightDirection[0]);

        // Upload tangent-space (view-space) light positions/directions used in fragment shader
        glUniform3fv(glGetUniformLocation(shaderID, ("tangentSpaceLightPosition[" + idx + "]").c_str()), 1, &VSLightPosition[0]);
        glUniform3fv(glGetUniformLocation(shaderID, ("tangentSpaceLightDirection[" + idx + "]").c_str()), 1, &VSLightDirection[0]);

        // Upload full light struct data to shader
